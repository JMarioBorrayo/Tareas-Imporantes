...
export class TaskserviceService {
  ...
  constructor() {
   ...
   public removeImportant(pos:number){
    this.importantTasks.splice(pos, 1);
   }

   public importantTask(pos: number){
    this.importantTasks.push(this.tasks[pos]) //agrega la tarea a tareas importante
    this.removeTask(pos);
   }

   public importantDone(pos: number){
    this.completedTasks.push(this.importantTasks[pos]) //agrega la tarea a tareas importantes
    this.removeImportant(pos);
   }

   public getImportantTasks(){
    return this.importantTasks; //regresa las tareas importantes
  }
